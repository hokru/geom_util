...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 ...
 10   continue
      ...
      r=dsqrt(r/(n-1))
      write(*,'(i4,''  rms '',d14.6,''   norm g'',d14.6)') it, r, gn
      ...
      do i=1,n
         call pldist(i,x,y,z,s,t,u,rx)
         write(*,'(i4,''.atom, dev (ang.)'',f14.6)') i,rx
         sum1=sum1+    rx 
         sum2=sum2+abs(rx)
      enddo

      write(*,'(/''mean     dev (ang.)'',f14.6)') sum1/n
      write(*,'(/''mean abs dev (ang.)'',f14.6)') sum2/n

      f=1./bohr
      write(*,'(/'' plane coordinates (au)''/)')
      write(*,'('' x, y, z'',3f14.6)') s(1)*f,t(1)*f,u(1)*f
      write(*,'('' x, y, z'',3f14.6)') s(2)*f,t(2)*f,u(2)*f
      write(*,'('' x, y, z'',3f14.6)') s(3)*f,t(3)*f,u(3)*f

      call febene(s,t,u,aa,bb,cc,dd)

      write(*,'(/'' plane parameters A B C D'',4F14.6/)') aa,bb,cc,dd
